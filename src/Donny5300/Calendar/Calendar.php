...
//                dd($this->todayClass);
    ...
    public function setWeekSelect($min = 5, $max = 5, $class = false)
    {
        ...
        $nWeeks = Carbon::now()->addWeeks($min);
        $pWeeks = Carbon::now()->subWeeks($max);
        $html = '<select>';
        $counter = 0;
        for ($week = $pWeeks; $week <= $nWeeks; $week->addWeek()) {

            if ($this->format_date($week->toDateString(), 'W') == $this->weekNumber) {
                $html .= '<option value="' . $week->toDateString() . '" selected="selected">' .
                    $this->format_date($week->toDateString(), 'W')
                    . ' (Huidige)</option>';
            } else {
                $html .= '<option value="' . $week->toDateString() . '">' . $this->format_date($week->toDateString(), 'W') . '</option>';
            }
        }
        $html .= '</select>';
            ...
            foreach($options as $option){
                switch($option){
                    case 'views';
                        ...
                        $this->eventView = Config::get($package.'views.table_cell');
                        break;
                    ...
                    case 'today-class':
                        $this->todayClass = 'class="' . Config::get($package.'today-class') . '"';
                    break;
                    ...
                    case 'week-select':
                        ...
                        $prev = Config::get($package.'view_mode.week-select.next');
